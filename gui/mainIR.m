...
app.setArmControllerObj(ctrlDobot);  % or ctrlTM5, depending on which one you want to control
app.setArmControllerObj1(ctrlTM5);  % or ctrlTM5, depending on which one you want to control
...
for i = 1:length(basketArray(1,:))
    ...
    plot3(x,y,z,'|-r');
    basketArray{2,i} = PlaceObject('baskets.ply', [x,y,z]);
end
...
for i = 1:length(ballArray(1,:))
    ...
    z = ballArray{1,i}(3,4);
    plot3(x,y,z,'|-g');
    ballArray{2,i} = PlaceObject('apple.ply', [x,y,z]);
end
...
emergency = PlaceObject('emergencyStopWallMounted.ply', [-1,-1.85,1.5]);
extinguisher = PlaceObject('fireExtinguisher.ply', [-1.5, -1.8, 0]);

table = PlaceObject('table.ply',[0,0,0]);
rotate(table,   [0, 0, 1], 90);

wall = PlaceObject('Wall.ply', [-1.99,-1.99,-1]);
wall1 = PlaceObject('Wall.ply', [-1.99,-1.99,-1]);
rotate(wall1,   [0, 0, 1], 90);

fence = PlaceObject('barrier1.5x0.2x1m.ply', [0.7, 1.5, 0]);
fence1 = PlaceObject('barrier1.5x0.2x1m.ply', [0.7, -1.3, 0]);
fence2 = PlaceObject('barrier1.5x0.2x1m.ply', [0.8, -1.55, 0]);
fence3 = PlaceObject('barrier1.5x0.2x1m.ply', [0.8, 1.5, 0]);
fence4 = PlaceObject('barrier1.5x0.2x1m.ply', [-0.7, 1.5, 0]);
fence5 = PlaceObject('barrier1.5x0.2x1m.ply', [-0.7, -1.3, 0]);
fence6 = PlaceObject('barrier1.5x0.2x1m.ply', [-0.5, -1.55, 0]);
fence7 = PlaceObject('barrier1.5x0.2x1m.ply', [-0.5, 1.5, 0]);
...
for i = 1:GetPairCount
    ...
    for j = 1:stageMax
        % ctrlTM5.Estop(EstopTrigger)
        % ctrlDobot.Estop(EstopTrigger)
