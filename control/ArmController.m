...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function self = ArmController(robot,stepsPerMetre,ikErrorMax,velocityMax,qInit)
            ...
            if nargin > 0 % Just robot arm passed
                ...
                if nargin > 3 % Arm, steps, inv kinematic error max and velocity max
                    self.velocityMax = velocityMax;
                end
                if nargin > 4 % All elements passed
...
%-------------------------------------------------------------------------%
        function self = setNextPose(self,nextPose)
            % Appending current pose to prevPoses array
            prevPoses = self.previousPoses;
            prevPoses{end+1} = self.currentPose;
...
%-------------------------------------------------------------------------%
        function self = createWaypoint(self,desiredTr,currentScale)
            distance = desiredTr-self.robot.model.getPos();
            maxSteps = self.stepsPerMetre*distance;

            qPath = jtraj()
        end
%-------------------------------------------------------------------------%
        function self = moveToNextPoint(self,qPath,mask)
            self.robot.model.Ikcon(qPath,mask);
        end
%-------------------------------------------------------------------------%
    end
end
